...
export default function KratosRecoveryPage() {
  ...
  useEffect(() => {
    if (!flowId) {
      window.location.href = "/api/.ory/self-service/recovery/browser";
      return;
    }
    frontend
      ...
      .catch((err) => {
        console.error("Fatal Error fetching Kratos recovery flow:", err);
        document.body.innerHTML = `
          <div style="padding: 2rem; background: #220000; color: #ff9999; min-height: 100vh; font-family: monospace;">
            <h1 style="font-size: 1.5rem; font-weight: bold; margin-bottom: 1rem;">Kratos Failed to Load the Recovery Flow</h1>
  ...
  const handleSubmit = async (e: React.SyntheticEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (err.response?.status === 422) {
        // Kratos signals a browser redirect is required (e.g. to a settings flow to set new password)
