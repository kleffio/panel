...
export default function KratosSettingsPage() {
  ...
  useEffect(() => {
    if (!flowId) {
      window.location.href = "/api/.ory/self-service/settings/browser";
      return;
    }
    frontend
      ...
      .catch((err) => {
        console.error("Fatal Error fetching Kratos settings flow:", err);
        document.body.innerHTML = `
          <div style="padding: 2rem; background: #220000; color: #ff9999; min-height: 100vh; font-family: monospace;">
            <h1 style="font-size: 1.5rem; font-weight: bold; margin-bottom: 1rem;">Kratos Failed to Load the Settings Flow</h1>
