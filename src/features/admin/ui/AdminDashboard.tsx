...
const roleOptions = ["Admin", "Finance", "Support"] as const;
const requestTypeFilters = ["All", "Access", "Capacity", "Finance"] as const;
...
const incidentActions = [
  {
    ...
    hoverDescription: "Pause sensitive admin mutations until the incident is contained.",
  ...
  },
  {
    ...
    hoverDescription: "Open a new incident record so the team can track and respond to the issue.",
  ...
  },
  {
    ...
    hoverDescription: "Download a snapshot of current security posture, controls, and audit health.",
    tone: "outline" as const,
...
function memberStatusVariant(status: AdminMember["status"]): "secondary" | "outline" | "destructive" {
  if (status === "Active") return "secondary";
  if (status === "Pending review") return "outline";
  return "destructive";
}

function auditTone(severity: OrgSettingsData["auditTrail"][number]["severity"]) {
  if (severity === "critical") return "border-l-rose-500";
  if (severity === "warning") return "border-l-amber-400";
  return "border-l-sky-500";
}

function requestStatusVariant(status: AdminRequest["status"]): "secondary" | "outline" | "destructive" {
...
export function AdminDashboard({ initialData }: { initialData: OrgSettingsData }) {
  ...
  const [auditWebhook, setAuditWebhook] = React.useState("https://hooks.kleff.io/audit/admin");
  const [pendingIncidentAction, setPendingIncidentAction] = React.useState<(typeof incidentActions)[number] | null>(null);
  const [activeTab, setActiveTab] = React.useState("overview");
  const [requestTypeFilter, setRequestTypeFilter] = React.useState<(typeof requestTypeFilters)[number]>("All");
  const [requestStatusFilter, setRequestStatusFilter] = React.useState<(typeof requestStatusFilters)[number]>("All");

  const seatUsage = Math.round((initialData.seatsUsed / initialData.seatsTotal) * 100);
  ...
  const handleMemberAction = (member: AdminMember, action: "Review access" | "Reset session" | "Suspend") => {
    if (action === "Suspend") {
      setMembers((current) =>
        current.map((entry) =>
  ...
  const handleRequestAction = (request: AdminRequest, status: "Approved" | "Rejected") => {
    setRequests((current) =>
      current.map((entry) =>
    ...
    <div className="mx-auto flex w-full max-w-7xl flex-col gap-6 animate-in fade-in duration-500">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <Button
            variant="outline"
            onClick={() => handleQuickAction("Directory sync started", "Provisioning changes will settle in a few seconds.")}
          >
            <Sparkles />
            Sync Directory
          </Button>
          ...
          <Button onClick={handleSave} disabled={isPending || (name === initialData.orgName && ramLimit === initialData.ramLimit)}>
      ...
      <Alert variant="warning">
        ...
        <AlertDescription>
          Two operators still do not have MFA enabled, and one RAM increase request is waiting on approval.
        </AlertDescription>
      </Alert>
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        <Card>
          ...
          <CardContent>
            <p className="text-sm text-muted-foreground">Session hardening, MFA coverage, and audit hooks are healthy.</p>
          </CardContent>
        </Card>
        <Card>
          <CardHeader>
          ...
          <CardContent className="space-y-2">
            ...
            <p className="text-sm text-muted-foreground">Four seats remain before role provisioning slows down.</p>
          </CardContent>
        </Card>
        <Card>
          <CardHeader>
            <CardDescription>Active projects</CardDescription>
            <CardTitle className="text-3xl text-foreground">{initialData.activeProjects}</CardTitle>
          </CardHeader>
          <CardContent>
            <p className="text-sm text-muted-foreground">Three deployments are currently consuming shared admin limits.</p>
          </CardContent>
        </Card>
        <Card>
          <CardHeader>
            ...
            <Card>
              <CardHeader>
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <CardDescription>Core settings and billing contact for the workspace.</CardDescription>
                  </div>
                </div>
                <CardAction>
                  <Button size="sm" variant="ghost" onClick={() => handleQuickAction("Profile snapshot captured", "A secure change log entry has been created.")}>
                    Snapshot
                  </Button>
                </CardAction>
              </CardHeader>
              <CardContent className="grid gap-4 md:grid-cols-2">
                ...
                <div className="space-y-2">
                  ...
                  <Input id="billing-email" type="email" value={initialData.billingEmail} readOnly />
                </div>
                <div className="space-y-2">
              ...
              <CardHeader>
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <CardDescription>Shared memory ceiling and automation toggles.</CardDescription>
                  </div>
                </div>
              </CardHeader>
              <CardContent className="space-y-5">
                <div className="space-y-2">
                  ...
                  <input
                    ...
                    step="8"
                    value={ramLimit}
                    onChange={(event) => setRamLimit(Number(event.target.value))}
                  ...
                  <div className="rounded-lg border border-border bg-muted/40 p-3">
                    <div className="mb-2 flex items-center justify-between text-sm">
                      ...
                      <span className="text-muted-foreground">{initialData.ramUsed} / {ramLimit} GB</span>
                    </div>
                ...
                <div className="space-y-3">
                  <div className="flex items-center justify-between rounded-lg border border-border p-3">
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">Freeze deployments while policy changes roll out.</p>
                    </div>
          ...
          <div className="grid gap-4 xl:grid-cols-[0.85fr_1.15fr]">
            <Card>
              <CardHeader>
                ...
                <CardDescription>People who have been staged but have not accepted access yet.</CardDescription>
              </CardHeader>
                ...
                {initialData.pendingInvites.map((invite) => (
                  <div key={invite.id} className="rounded-lg border border-border p-3">
                    ...
                    <div className="mt-3 flex gap-2">
                      <Button size="xs" variant="outline" onClick={() => handleQuickAction("Invite resent", `A fresh link was sent to ${invite.email}.`)}>
                        Resend
                      </Button>
                      <Button size="xs" variant="ghost" onClick={() => handleQuickAction("Invite revoked", `${invite.email} was removed from the queue.`)}>
                        Revoke
                      </Button>
                    </div>
                  </div>
            ...
            <Card>
              ...
              <CardContent className="space-y-3">
                {initialData.auditTrail.map((event) => (
                  <div key={event.id} className={`rounded-lg border border-border border-l-4 p-3 ${auditTone(event.severity)}`}>
                    <div className="flex items-start justify-between gap-3">
                      <div>
        ...
        <TabsContent value="access" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Review roles, check MFA, and take action on individual operators.</CardDescription>
            </CardHeader>
            <CardContent>
              <Table>
                <TableHeader>
                      ...
                      <TableCell className="text-right">
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align="end">
                            ...
                            <DropdownMenuSeparator />
                            <DropdownMenuItem onClick={() => handleMemberAction(member, "Review access")}>
                              Review access
                            </DropdownMenuItem>
                            <DropdownMenuItem onClick={() => handleMemberAction(member, "Reset session")}>
                              Reset session
                            </DropdownMenuItem>
                            <DropdownMenuItem variant="destructive" onClick={() => handleMemberAction(member, "Suspend")}>
                              Suspend
                            </DropdownMenuItem>
                          </DropdownMenuContent>
                        </DropdownMenu>
          ...
          <div className="grid gap-4 lg:grid-cols-3">
            ...
            <Card>
              ...
              <CardContent className="space-y-2">
                <Button className="w-full justify-start" onClick={() => handleQuickAction("Access review started", "Quarterly review workspace opened.")}>
                  <Users2 />
                  Run access review
                </Button>
                <Button className="w-full justify-start" variant="outline" onClick={() => handleQuickAction("Session reset staged", "All non-owner admin sessions will refresh.")}>
                  <Zap />
                  Refresh admin sessions
                </Button>
                <Button className="w-full justify-start" variant="secondary" onClick={() => handleQuickAction("Webhook test sent", "Audit sink acknowledged the payload.")}>
                  <ShieldCheck />
                  Test audit webhook
                </Button>
              </CardContent>
        ...
        <TabsContent value="requests" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Review admin requests with quick filters before approving or rejecting them.</CardDescription>
            </CardHeader>
                ...
                <div className="flex flex-wrap gap-2">
                  {requestTypeFilters.map((filter) => (
                    <Button
                      key={filter}
                      size="sm"
                      variant={requestTypeFilter === filter ? "default" : "outline"}
                      onClick={() => setRequestTypeFilter(filter)}
                    >
                      {filter}
                    </Button>
                  ))}
                ...
                <div className="flex flex-wrap gap-2">
                  {requestStatusFilters.map((filter) => (
                    <Button
                      key={filter}
                      size="sm"
                      variant={requestStatusFilter === filter ? "secondary" : "outline"}
                        ...
                        <TableCell className="text-right">
                          {request.status === "Pending" ? (
                            <div className="flex justify-end gap-2">
                              <Button size="sm" variant="outline" onClick={() => handleRequestAction(request, "Rejected")}>
                                Reject
                              </Button>
                              <Button size="sm" onClick={() => handleRequestAction(request, "Approved")}>
                                Approve
                              </Button>
                            </div>
                          ) : (
        ...
        <TabsContent value="security" className="space-y-4">
          <div className="grid gap-4 xl:grid-cols-[0.95fr_1.05fr]">
            <Card>
              <CardHeader>
                ...
                <CardDescription>Hardening toggles for admin and platform controls.</CardDescription>
              </CardHeader>
              <CardContent className="space-y-3">
                <div className="flex items-center justify-between rounded-lg border border-border p-3">
                  <div>
                    <p className="text-sm font-medium text-foreground">Session lockdown</p>
                    <p className="text-xs text-muted-foreground">Force step-up verification on sensitive changes.</p>
                  </div>
                  <Switch checked={sessionLockdown} onCheckedChange={setSessionLockdown} />
                </div>
                <div className="flex items-center justify-between rounded-lg border border-border p-3">
                  <div>
                    <p className="text-sm font-medium text-foreground">Block unverified finance access</p>
                    <p className="text-xs text-muted-foreground">Require MFA before invoice or payout actions are visible.</p>
                  </div>
                  <Switch checked={true} onCheckedChange={() => handleQuickAction("Policy confirmed", "Finance scope already requires verified access.")} />
                </div>
                <div className="space-y-2 rounded-lg border border-border p-3">
                  <label className="text-sm font-medium text-foreground" htmlFor="audit-webhook">
                    Audit webhook
                  </label>
                  <Input
                    id="audit-webhook"
                    value={auditWebhook}
                    onChange={(event) => setAuditWebhook(event.target.value)}
                  />
                  <div className="flex gap-2">
                    <Button size="sm" variant="outline" onClick={() => handleQuickAction("Webhook verified", "The endpoint responded with a signed acknowledgement.")}>
                      Verify endpoint
                    </Button>
                    <Button size="sm" variant="ghost" onClick={() => setAuditWebhook("https://hooks.kleff.io/audit/admin")}>
                      Restore default
                    </Button>
                  </div>
                </div>
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>Fast actions when the admin plane needs to be tightened quickly.</CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                <Alert variant="info">
                  <ShieldCheck className="size-4" />
                  <AlertTitle>Audit delivery is healthy</AlertTitle>
                  <AlertDescription>Last signed event reached your sink 43 seconds ago.</AlertDescription>
                </Alert>
                <div className="grid gap-2 sm:grid-cols-2">
                  {incidentActions.map((action) => (
                    <HoverCard key={action.buttonLabel} openDelay={120}>
                      <HoverCardTrigger asChild>
                        <Button variant={action.tone} onClick={() => setPendingIncidentAction(action)}>
                          {action.buttonLabel}
                        </Button>
                      </HoverCardTrigger>
                      <HoverCardContent align="start">
                        <p className="font-medium text-foreground">{action.buttonLabel}</p>
                        <p className="mt-1 text-xs text-muted-foreground">{action.hoverDescription}</p>
                      </HoverCardContent>
                    </HoverCard>
                  ))}
                </div>
                <AlertDialog open={pendingIncidentAction !== null} onOpenChange={(open) => !open && setPendingIncidentAction(null)}>
                  <AlertDialogContent size="sm">
                    ...
                    <AlertDialogFooter>
                      ...
                      <AlertDialogAction
                        variant={pendingIncidentAction?.tone === "destructive" ? "destructive" : "default"}
                        onClick={() => {
                          if (!pendingIncidentAction) return;
                          handleQuickAction(pendingIncidentAction.successLabel, pendingIncidentAction.successDescription);
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Small changes that would improve operational resilience quickly.</CardDescription>
            </CardHeader>
            <CardContent className="grid gap-3 md:grid-cols-3">
              <div className="rounded-lg border border-border p-4">
                <p className="text-sm font-medium text-foreground">Require MFA for finance</p>
                <p className="mt-1 text-xs text-muted-foreground">One finance-scoped operator is still missing a second factor.</p>
              </div>
              <div className="rounded-lg border border-border p-4">
                <p className="text-sm font-medium text-foreground">Tighten RAM approvals</p>
                <p className="mt-1 text-xs text-muted-foreground">Current policy allows same-day changes without owner sign-off.</p>
              </div>
              <div className="rounded-lg border border-border p-4">
                <p className="text-sm font-medium text-foreground">Expand owner coverage</p>
                <p className="mt-1 text-xs text-muted-foreground">A second owner would reduce incident escalation bottlenecks.</p>
              </div>
            </CardContent>
          </Card>
        </TabsContent>
